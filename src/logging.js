class Logger {
  ...
  logFactory(level, type, logData) {
    const labels = { component: this.config.logging.source, level: level, type: type };
    const cleanData = this.sanitize(logData);
  ...
  async sendLogToGrafana(event) {
    ...
    try {
      const res = await fetch(`${this.config.logging.endpointUrl}`, {
        method: 'post',
        body: body,
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${this.config.logging.accountId}:${this.config.logging.apiKey}`,
        },
      });
      if (!res.ok) {
